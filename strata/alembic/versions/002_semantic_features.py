...
def upgrade() -> None:
    ...
    op.add_column("document", sa.Column("version_number", sa.Integer(), nullable=True, server_default="1"))
    op.add_column("document", sa.Column("previous_version_id", sa.UUID(), nullable=True))
    op.add_column("document", sa.Column("doc_type", sa.Enum("CONTRACT", "POLICY", "RFC", "OTHER", name="doctype", create_type=False), nullable=True))
    ...
    op.create_table(
        ...
        sa.Column("interaction_type", sa.Text(), nullable=False),  # search_chunks, answer_with_evidence, etc.
    ...
    op.create_table(
        ...
        sa.Column("view_type", sa.Text(), nullable=False, server_default="raw"),  # raw, redacted, summary
